...
为中文文章设计和生成 16:9 横版正文配图。目标不是做商业插画、PPT 信息图或可爱卡通，而是把文章里的关键判断、流程、结构、状态或隐喻，变成一张配图。

默认视觉 IP 是"Kero"：留着中分及耳中短发的年轻男生，带着温和的微笑。标志性的大黑圆眼、简化的点状鼻子、弧线嘴巴。穿着波浪花纹毛衣和牛仔夹克。Kero 必须参与画面的核心动作，不能只是站在旁边当装饰。
...
- `references/kero-ip.md`：Kero IP 的形象、性格、动作库和禁忌。
...
不要平均配图。优先选择"认知锚点"，例如：核心判断、两个断点、输入输出闭环、分流、前后对比、一鱼多吃、承接路径、常见坑、角色状态变化、系统内部黑盒、方法论分层等。
...
不要复刻过往案例。案例只提供风格密度和 Kero 参与方式，不能直接复用已有构图。
